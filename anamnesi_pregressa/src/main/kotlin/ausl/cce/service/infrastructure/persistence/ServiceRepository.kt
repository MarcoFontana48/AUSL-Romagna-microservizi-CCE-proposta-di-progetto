...
class MongoAllergyIntoleranceRepository(
    ...
    credentials = credentials,
    collectionName = "allergy_intolerance",
    entitySerializer = { entity ->
        Document().apply {
            append("entityId", entity.id.value)
            append("allergyIntoleranceJson", entity.toJson())
            append("patientReference", entity.allergyIntolerance.patient.reference)
        }
    },
    entityDeserializer = { document ->
        try {
            val entityId = document.getString("entityId") ?: document.getString("_id")
            val allergyIntoleranceJson = document.getString("allergyIntoleranceJson")
            if (entityId != null && allergyIntoleranceJson != null) {
                val allergyIntolerance = allergyIntoleranceJson.fromJsonToAllergyIntolerance()
                AllergyIntoleranceEntity.of(AllergyIntoleranceId(entityId), allergyIntolerance)
            } else null
        } catch (e: Exception) {
            null
        }
