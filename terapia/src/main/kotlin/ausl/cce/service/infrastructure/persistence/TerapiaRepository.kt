...
class MongoCarePlanRepository(
    ...
    credentials = credentials,
    collectionName = "care_plan",
    entitySerializer = { entity ->
        Document().apply {
            append("entityId", entity.id.value)
            append("carePlanJson", entity.toJson())
            append("subjectReference", entity.carePlan.subject.reference)
            append("title", entity.carePlan.title)
            append("status", entity.carePlan.status?.toCode())
            append("intent", entity.carePlan.intent?.toCode())
        }
    },
    entityDeserializer = { document ->
        try {
            val entityId = document.getString("entityId") ?: document.getString("_id")
            val carePlanJson = document.getString("carePlanJson")
            if (entityId != null && carePlanJson != null) {
                val carePlan = carePlanJson.fromJsonToCarePlan()
                CarePlanEntity.of(CarePlanId(entityId), carePlan)
            } else null
        } catch (e: Exception) {
            null
        }
